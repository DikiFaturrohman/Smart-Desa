...
<section class="section">
    ...
    @if(empty($sktm->no_surat) && Auth::user()->roles()->first()->id != 'operator')
    ...
    @elseif($sktm->verifikasi_kasi == '0' &&$sktm->verifikasi_sekdes == '0' && !empty($sktm->no_surat) &&
    ...
    @elseif($sktm->verifikasi_sekdes == '0' && $sktm->verifikasi_kasi == '1' && Auth::user()->roles()->first()->id !=
    'sekretaris_desa')
    <div class="alert alert-warning alert-dismissible show fade">
        <div class="alert-body">
            <button class="close" data-dismiss="alert">
    ...
    @elseif($sktm->verifikasi_sekdes == '1' && $sktm->verifikasi_kades == '0' && Auth::user()->roles()->first()->id ==
    'operator')
    <div class="alert alert-warning alert-dismissible show fade">
        <div class="alert-body">
            <button class="close" data-dismiss="alert">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <a href="{{asset('backend/images/dokumen/sktm/rtrw/'.$sktm->file_sp_rtrw)}}"
                                    target="_blank">
                                    <img src="{{asset('backend/images/dokumen/sktm/rtrw/'.$sktm->file_sp_rtrw)}}"
                                        width="200px"></a>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <a href="{{asset('backend/images/dokumen/sktm/ktp/'.$sktm->file_ktp)}}"
                                    target="_blank"><img
                                        src="{{asset('backend/images/dokumen/sktm/ktp/'.$sktm->file_ktp)}}"
                                        width="200px"></a>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <a href="{{asset('backend/images/dokumen/sktm/kk/'.$sktm->file_kk)}}" target="_blank"
                                    width="200px"><img src="{{asset('backend/images/dokumen/sktm/kk/'.$sktm->file_kk)}}"
                                        width="200px"></a>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <a href="{{asset('backend/images/dokumen/sktm/surat_pernyataan/'.$sktm->file_surat_pernyataan)}}"
                                    target="_blank" width="200px"><img
                                        src="{{asset('backend/images/dokumen/sktm/surat_pernyataan/'.$sktm->file_surat_pernyataan)}}"
                                        width="200px"></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer text-right">
                        ...
                        @if(Session::get('permission')->update == 1 && $sktm->status == '1' && empty($sktm->no_surat) &&
                        Auth::user()->roles()->first()->id == 'operator')
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#rejectedOperator"
                            data-id="{{$sktm->encodeHash($sktm->id)}}" class="operator btn btn-md btn-danger btn-icon"
                            title="Tolak">Tolak</a>
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#acceptedOperator"
                            data-id="{{$sktm->encodeHash($sktm->id)}}" class="operator btn btn-md btn-success btn-icon"
                            title="Terima">Terima</a>
                        @elseif(!empty($sktm->no_surat) && Auth::user()->roles()->first()->id == 'kasi' &&
                        $sktm->kasi_id == Auth::guard('admin')->user()->id)
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#verifikasiKasi"
                            data-id="{{$sktm->encodeHash($sktm->id)}}" class="kasi btn btn-md btn-success btn-icon"
                            title="Verifikasi">Verifikasi</a>
                        @elseif(!empty($sktm->no_surat) && Auth::user()->roles()->first()->id == 'sekretaris_desa' &&
                        $sktm->verifikasi_kasi == '1'&&
                        $sktm->verifikasi_sekdes == '0')
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#verifikasiSekdes"
                            data-id="{{$sktm->encodeHash($sktm->id)}}" class="sekdes btn btn-md btn-success btn-icon"
                            title="Verifikasi">Verifikasi</a>
                        @elseif(!empty($sktm->no_surat) && Auth::user()->roles()->first()->id == 'kepala_desa' &&
                        $sktm->verifikasi_kasi
                        == '1' && $sktm->verifikasi_sekdes == '1' && $sktm->verifikasi_kades == '0')
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#verifikasiKades"
                            data-id="{{$sktm->encodeHash($sktm->id)}}" class="kades btn btn-md btn-success btn-icon"
                            title="Verifikasi">Verifikasi</a>
                        @endif
                    </div>
                </div>
...
<form action="{{route('backend.dokumen.sktm.reject')}}" method="post">
    {{csrf_field()}}
...
<form action="{{route('backend.dokumen.sktm.accept')}}" method="post">
    {{csrf_field()}}
    <input type="hidden" class="id" value="{{old('id')}}" name="id">
...
<form action="{{route('backend.dokumen.sktm.kades')}}" method="post">
    {{csrf_field()}}
    <input type="hidden" class="id" value="{{old('id',$sktm->encodeHash($sktm->id))}}" name="id">
...
<form action="{{route('backend.dokumen.sktm.sekdes')}}" method="post">
    {{csrf_field()}}
...
<form action="{{route('backend.dokumen.sktm.kasi')}}" method="post">
    {{csrf_field()}}
    <input type="hidden" class="id" value="" name="id">
