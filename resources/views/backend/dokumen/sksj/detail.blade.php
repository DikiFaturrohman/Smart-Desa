...
<section class="section">
    ...
    @if(empty($sksj->no_surat) && Auth::user()->roles()->first()->id != 'operator')
    ...
    @elseif($sksj->verifikasi_kasi == '0' &&$sksj->verifikasi_sekdes == '0' &&  !empty($sksj->no_surat) && Auth::user()->roles()->first()->id != 'kasi')
    <div class="alert alert-warning alert-dismissible show fade">
        <div class="alert-body">
            <button class="close" data-dismiss="alert">
                <span>&times;</span>
    ...
    @elseif($sksj->verifikasi_sekdes == '0' && $sksj->verifikasi_kasi == '1' && Auth::user()->roles()->first()->id != 'sekretaris_desa')
    <div class="alert alert-warning alert-dismissible show fade">
        <div class="alert-body">
            <button class="close" data-dismiss="alert">
                <span>&times;</span>
    ...
    @elseif($sksj->verifikasi_sekdes == '1' && $sksj->verifikasi_kades == '0' && Auth::user()->roles()->first()->id == 'operator')
    <div class="alert alert-warning alert-dismissible show fade">
        <div class="alert-body">
            <button class="close" data-dismiss="alert">
                <span>&times;</span>
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-footer text-right">
                        ...
                        @if(Session::get('permission')->update == 1 && $sksj->status == '1' && empty($sksj->no_surat) &&
                        Auth::user()->roles()->first()->id == 'operator')
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#rejectedOperator"
                            data-id="{{$sksj->encodeHash($sksj->id)}}" class="operator btn btn-md btn-danger btn-icon"
                            title="Tolak">Tolak</a>
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#acceptedOperator"
                            data-id="{{$sksj->encodeHash($sksj->id)}}" class="operator btn btn-md btn-success btn-icon"
                            title="Terima">Terima</a>
                        @elseif(!empty($sksj->no_surat) && Auth::user()->roles()->first()->id == 'kasi' && $sksj->kasi_id == Auth::guard('admin')->user()->id)
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#verifikasiKasi"
                            data-id="{{$sksj->encodeHash($sksj->id)}}" class="kasi btn btn-md btn-success btn-icon"
                            title="Verifikasi">Verifikasi</a>
                        @elseif(!empty($sksj->no_surat) && Auth::user()->roles()->first()->id == 'sekretaris_desa' &&
                        $sksj->verifikasi_kasi == '1'&&
                        $sksj->verifikasi_sekdes == '0')
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#verifikasiSekdes"
                            data-id="{{$sksj->encodeHash($sksj->id)}}" class="sekdes btn btn-md btn-success btn-icon"
                            title="Verifikasi">Verifikasi</a>
                        @elseif(!empty($sksj->no_surat) && Auth::user()->roles()->first()->id == 'kepala_desa' &&
                        $sksj->verifikasi_kasi
                        == '1' && $sksj->verifikasi_sekdes == '1' && $sksj->verifikasi_kades == '0') 
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#verifikasiKades"
                            data-id="{{$sksj->encodeHash($sksj->id)}}" class="kades btn btn-md btn-success btn-icon"
                            title="Verifikasi">Verifikasi</a>
                        @endif
                    </div>
                </div>
...
<form action="{{route('backend.dokumen.sksj.reject')}}" method="post">
    {{csrf_field()}}
...
<form action="{{route('backend.dokumen.sksj.accept')}}" method="post">
    {{csrf_field()}}
    <input type="hidden" class="id" value="{{old('id')}}" name="id">
...
<form action="{{route('backend.dokumen.sksj.kades')}}" method="post">
    {{csrf_field()}}
    <input type="hidden" class="id" value="{{old('id',$sksj->encodeHash($sksj->id))}}" name="id">
...
<form action="{{route('backend.dokumen.sksj.sekdes')}}" method="post">
    {{csrf_field()}}
...
<form action="{{route('backend.dokumen.sksj.kasi')}}" method="post">
    {{csrf_field()}}
    <input type="hidden" class="id" value="" name="id">
