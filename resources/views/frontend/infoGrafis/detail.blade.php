...
@section('title') {{$grafis->title}} @endsection

@section('meta')
<meta name="description" content="{{htmlspecialchars($grafis->content)}}" />
<meta name="keywords" content="{{htmlspecialchars($grafis->title)}}" />
<meta property="og:title" content="{{$grafis->title}} "/>
<meta property="og:type" content="infografis"/>
<meta property="og:image" content="{{($grafis->img)?asset('backend/images/informasi/infoGrafis/'.$grafis->img):asset('backend/images/default.jpg')}}"/>
@endsection
 ...
 -->
                ...
                <div class="flex-wr-s-s p-b-10">
                    <span class="f1-s-3 m-r-15">
                        <span>
                            <i class="fa fa-user"></i> {{$grafis->created_by}} |&nbsp;
                        </span>
                        <span>
                            <i class="fa fa-calendar-alt"></i>
                            {{\Carbon\Carbon::parse($grafis->created_at)->translatedFormat('d F Y')}} |&nbsp;
                        </span>
                        <span>
                            <i class="fa fa-eye"></i> {{$grafis->hit}} Views |&nbsp;
                        </span>
                        <span>
            ...
            <div class="detail-content pb-4">
                ...
                <img src="{{asset('backend/images/informasi/infoGrafis/'.$grafis->img)}}" alt="{{$grafis->title}}"
            ...
            <div class="pb-2 mb-2">
                ...
                <form method="post" action="{{route('frontend.komentar')}}">
                    @csrf
                    <div class="row pb-2">
                        <input type="hidden" name="info_id" value="{{$grafis->id}}">
									...
									<div class="card-body">
											<div class="row">
													<div class="col-md-2">
															...
															<p class="text-secondary text-center f1-s-5">{{\Carbon\Carbon::parse($data->created_at)->diffForHumans()}}</p>
													</div>
													<div class="col-md-10 pl-3 pr-1">
															<p class="d-flex justify-content-between">
																	<a class="pt-0 mt-0"
																			href="#"><strong>{{$data->nama}}</strong></a>
																	<span class="f1-s-3"><i class="fa fa-calendar-alt"></i>&nbsp;{{\Carbon\Carbon::parse($data->created_at)->translatedFormat('d M Y')}}</span>
															</p>
																	...
																	<div class="body">
																			...
																			<div class="message">
																					<p class="cl5 small p-t-5 p-b-5">
																							<i class="fa fa-user"></i> {{$data->admin}}<br>
																							<i class="fa fa-calendar-alt"></i> {{\Carbon\Carbon::parse($data->updated_at)->diffForHumans()}}
																					</p>
																					<p class="cl4 f1-s-1">{!!$data->balas!!}</p>
																			</div>
																	</div>
															</div>
															@endif
                                ...
                                <div class="row">
                                    <div class="col-md-4 img-container">
                                        <img class="img-to-fit" width="100%"
                                            src="{{asset('backend/images/informasi/pengumuman/'.$tabpengumuman->img)}}"
                        ...
                        <div class="tab-pane active" id="berita" role="tabpanel">
                            ...
                            <div class="sidelist shadow">
                                <div class="row">
                                    <div class="col-md-4 img-container">
                                        <img class="img-to-fit" width="100%"
                                            src="{{asset('backend/images/informasi/berita/'.$tabberita->img)}}"
                                            alt="{{($tabberita->title)}}" />
                                    </div>
                                    <div class="col-md-7 px-2 py-2">
                                        <small class="f1-s-3">
