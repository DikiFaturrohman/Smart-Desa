			...
			<div class="table-responsive py-2">
				<div class="container">
					<table id="download" class="table table-striped table-bordered" style="width:100%">
						...
						<tbody>
							...
							@foreach($download as $data)
								<tr>
										...
										<td> <a href="{{route('frontend.download.detail',['slug'=>$data->slug])}}">{{$data->title}}</a></td>
										<td>{{\Carbon\Carbon::parse($data->created_at)->translatedFormat('d F Y')}}</td>
										<td align="center" valign="top">
											<a href="{{asset('backend/files/dokumen/download/'.$data->file)}}" class="btn btn-gelap"><i class="fas fa-download"></i></a>
										</td>
								</tr>
							@endforeach
						</tbody>
						<tfoot>
								<tr>
									<td colspan="4">{{$download->links('frontend.layout.pagination')}}</td>
								</tr>
						</tfoot>
					</table>
				</div>
