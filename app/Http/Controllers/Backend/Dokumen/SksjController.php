...

{
...

    {
        ...
        try{
            ...
            if(empty(Auth::user()->desa_id)){
                ...
                $data['sksj'] =SKSJ::where('desa_id',Auth::user()->desa_id)->orderBy('created_at','asc')->orderBy('status','asc')->get();
...

    {
        ...
        try{
            ...
            if($signDokumen == 'berhasil'){
                ...
                $log = $this->suketLogNotifikasi($sksj,'sksj','Verifikasi','Pengajuan Surat Keterangan Sapu Jagat Telah di verifikasi Oleh Kepala Desa','kades','terima');

                $admin = $this->getAdmin('operator',Session::get('desa_id'));

                $logAdmin = $this->logNotifikasiAdmin($admin,'Verifikasi','Verifikasi Surat Keterangan Sapu Jagat disetujui oleh kepala desa');
...

    {
        ...
        try{
            ...
            $log = $this->suketLogNotifikasi($sksj,'sksj','Verifikasi','Pengajuan Surat Keterangan Sapu Jagat Telah di verifikasi Oleh Sekretaris Desa','sekdes','terima');

            $admin = $this->getAdmin('kepala_desa',Session::get('desa_id'));

            $logAdmin = $this->logNotifikasiAdmin($admin,'Verifikasi','Verifikasi Surat Keterangan Sapu Jagat disetujui oleh sekretaris desa');
